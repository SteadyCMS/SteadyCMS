  ...
  <SimpleModal :title='props.title' size="xl" >
    <div class="flex flex-row">
      ...
      <div class="w-1/4 flex h-10 max-w-10 flex-col pl-6 pr-2">
        <h2 class="text-lg text-tint-10 font-semibold">Media Info</h2>
        <img :src="currentImage" :alt="currentImage" class="rounded my-2 w-auto h-auto" loading="lazy">
        <p class="text-tint-10 font-semibold text-sm break-words leading-tight">{{ currentImage.substr(currentImage.lastIndexOf('/') + 1) }}</p>
    ...
    <template #footer>
      <div class="flex flex-row justify-between w-full">
        <button @click="showUploadDialog" class="flex flex-row space-x-2 items-center py-2 px-4 border border-tint-10 text-tint-10 hover:text-tint-8 bg-white text-base font-semibold rounded-lg ease-in-out duration-300"> 
          <UploadIcon class="fill-tint-10 w-5 h-5 mr-1"/>
          Upload
        </button>
        <button @click="confirmModal({accepted: true, selected: selectedImages, selectedPath: selectedImagePath })" class="py-2 px-4 text-white hover:text-white/80 bg-black hover:bg-black text-base font-semibold rounded-lg ease-in-out duration-300"> 
          {{ props.acceptText }}
        </button>
      </div>
    </template>
